...
func (deps *Deps) RefreshHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		utils.RespondWithError(w, http.StatusUnauthorized, "refresh_error", "Missing refresh token", "", nil)
		return
	}
	...
	if blacklisted == "1" {
		...
		utils.RespondWithError(w, http.StatusUnauthorized, "token_reuse", "Token reuse detected", "", nil)
		return
	}

	tx, err := deps.DB.BeginTx(ctx, nil)
	if err != nil {
		utils.RespondWithError(w, http.StatusInternalServerError, "transaction_error", "Transaction failed on refresh endpoint", "", err)
		return
	}
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			...
			utils.RespondWithError(w, http.StatusUnauthorized, "invalid_refresh_token", "invalid refresh token", "", nil)
			return
		}
		utils.RespondWithError(w, http.StatusInternalServerError, "database_error", "Database error", "", err)
		return
	}
	...
	if session.RevokedAt.Valid {
		...
		utils.RespondWithError(w, http.StatusUnauthorized, "session_expires", "Session expired", "", nil)
		return
	}
	...
	if now.After(session.MaxExpiresAt) {
		...
		clearRefreshToken(w, r)
		_ = tx.Commit()
		utils.RespondWithError(w, http.StatusUnauthorized, "session_exceed", "Session lifetime exceeded", "", nil)
		return
	}
	...
	if err != nil {
		utils.RespondWithError(w, http.StatusInternalServerError, "token_error", "token generation failed", "", err)
		return
	}
	...
	accessToken, err := deps.JWT.Generate(session.UserID.String(), newSessionID.String())
	if err != nil {
		utils.RespondWithError(w, http.StatusInternalServerError, "jwt_error", "JWT generation failed", "", err)
		return
	}
...
func clearRefreshToken(w http.ResponseWriter, r *http.Request) {
	secureCookie := utils.ShouldUseSecureCookie(r)
	http.SetCookie(w, &http.Cookie{
		Name:     "refresh_token",
		Value:    "",
		Path:     "/",
